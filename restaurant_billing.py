...
lblTitle = Label(Tops, font=('arial', 30, 'bold'), text='Restaurant Billing System', bd=15, bg='black',
...
def TotalofUnit():
    ...
    Prices_Drinks = (Unit1 * 50) + (Unit2 * 45) + (Unit3 * 60) + (Unit4 * 35) + (Unit5 * 50) + (Unit6 * 40) + (Unit7 * 50) + (Unit8 * 50)

    Prices_Food = (Unit9 * 290) + (Unit10 * 150) + (Unit11 * 280) + (Unit12 * 250) + (Unit13 * 250) + (Unit14 * 250) + (Unit15 * 280) + (Unit16 * 280)



    DrinksPrices =  str('%.2f' % Prices_Drinks) + "RS"
    FoodsPrices =   str('%.2f' % Prices_Food) + "RS"
    Total_of_Food.set(FoodsPrices)
    Total_of_Drinks.set(DrinksPrices)
    SC =  str('%.2f' % 1.59) + "RS"
    ServiceCharge.set(SC)

    Sub_Total_of_Unit = str('%.2f'%(Prices_Drinks + Prices_Food + 1.59)) + "RS"
    SubTotal.set(Sub_Total_of_Unit)

    Tax = str('%.2f'%((Prices_Drinks + Prices_Food + 1.59) * 0.10)) + "RS"
    PaidTax.set(Tax)

    TT = ((Prices_Drinks + Prices_Food + 1.59) * 0.10)
    TC =  str('%.2f'%(Prices_Drinks + Prices_Food + 1.59 + TT)) + "RS"
    TotalCost.set(TC)
...
def Receipt():
    ...
    textReceipt.insert(END, 'Ice Tea:\t\t\t\t\t' + iced_tea.get()+'\n')
    ...
    textReceipt.insert(END, 'Pepsi:\t\t\t\t\t' + cobra.get()+'\n')
    textReceipt.insert(END, 'Fried Chicken:\t\t\t\t\t' + fried_chicken.get()+'\n')
    textReceipt.insert(END, 'Shawarma:\t\t\t\t\t' + kare_kare.get()+'\n')
    textReceipt.insert(END, 'Crispy Burger :\t\t\t\t\t' + crispy_pata.get()+'\n')
    textReceipt.insert(END, 'Chicken Burger:\t\t\t\t\t' + sinigang_baboy.get()+'\n')
    textReceipt.insert(END, 'Chinies Rice:\t\t\t\t\t' + sinigang_hipon.get()+'\n')
    textReceipt.insert(END, 'Pizza Fries:\t\t\t\t\t' + bicol_express.get()+'\n')
    textReceipt.insert(END, 'Club Sandwich:\t\t\t\t\t' + asparagus_tofu.get()+'\n')
    textReceipt.insert(END, 'Chicken Sandwich:\t\t\t\t\t' + chicken_sisig.get()+'\n')
...
IcedTea = Checkbutton(Drinks_Function, text='Ice Tea', variable=variable2, onvalue=1, offvalue=0, font=('arial', 16, 'bold'),
...
Cobra = Checkbutton(Drinks_Function, text='Pepsi', variable=variable8, onvalue=1, offvalue=0, font=('arial', 16, 'bold'),
...
KareKare = Checkbutton(Food_Function,text="Shawrama",variable=variable10,onvalue = 1, offvalue=0,
                        font=('arial',16,'bold'),bg='sky blue',command=foodsKareKare).grid(row=1,sticky=W)
CrispyPata = Checkbutton(Food_Function,text="Crispy Burger ",variable=variable11,onvalue = 1, offvalue=0,
                        font=('arial',16,'bold'),bg='sky blue',command=foodsCrispyPata).grid(row=2,sticky=W)
SinigangBaboy = Checkbutton(Food_Function,text="Chicken Burger ",variable=variable12,onvalue = 1, offvalue=0,
                        font=('arial',16,'bold'),bg='sky blue',command=foodsSinigangBaboy).grid(row=3,sticky=W)
SinigangHipon = Checkbutton(Food_Function,text="Chinies Rice ",variable=variable13,onvalue = 1, offvalue=0,
                        font=('arial',16,'bold'),bg='sky blue',command=foodsSinigangHipon).grid(row=4,sticky=W)
BicolExpress = Checkbutton(Food_Function,text="Pizza Fries ",variable=variable14,onvalue = 1, offvalue=0,
                        font=('arial',16,'bold'),bg='sky blue',command=foodsBicolExpress).grid(row=5,sticky=W)
AsparagusTofu = Checkbutton(Food_Function,text="Club Sandwich ",variable=variable15,onvalue = 1, offvalue=0,
                        font=('arial',16,'bold'),bg='sky blue',command=foodsAsparagusTofu).grid(row=6,sticky=W)
ChickenSisig = Checkbutton(Food_Function,text="Chicken Sandwich ",variable=variable16,onvalue = 1, offvalue=0,
